<?php
    ...
    if(isset($_POST['submit'])) {
      ...
      if(in_array($room,$data)) {
        ...
        if($floor == 1) {
          if($room <= 5) {
            $sqlInsert = "INSERT INTO `room-male`(first_name,last_name,street_address,city,region,postal_code,country,phone_number,start_move_in,floor,room,expire) VALUE('$first_name','$last_name','$street_address','$city','$region','$postal_code','$country','$phone_number','$start_move_in','$floor','$room','$expired')";

             $queryInsert = mysqli_query($db,$sqlInsert);

             if($dateNow >=  $expired && $dateNow <=  $expired  ) {
              $queryDelete = mysqli_query($db,"DELETE FROM `room-male` WHERE expire= '$dateNow'");
             }
